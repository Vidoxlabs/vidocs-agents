...
2.  **Shared UI**: UI components belong in `packages/ui`, not inside `apps/web` unless they are page-specific.
3.  **Strict Boundaries**: APIs do not import frontend code. Frontend does not import server-side Python code.
...
- **Updates Required**:
  ...
  2. Run `pnpm install`.
  3. Update imports in `apps/web` to `import { Button } from "@vidoxlabs/ui"`.
